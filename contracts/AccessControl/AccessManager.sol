...
contract AccessManager {
    ...
    constructor(address _owner) {
        ...
        roleData.grantRole(_owner, AuthLib.Rank.PREATORMAXIMUS);
    }
    ...
    function grantRole(address account, AuthLib.Rank rank) public {
        ...
        if (rank == AuthLib.Rank.PREATORMAXIMUS) {
            require(roleData.getHighestRankForAccount(msg.sender) == AuthLib.Rank.PREATORMAXIMUS, "Insufficient privileges to grant roles.");
            ...
            require(roleData.getHighestRankForAccount(msg.sender) == AuthLib.Rank.PREATORMAXIMUS || roleData.getHighestRankForAccount(msg.sender) == AuthLib.Rank.CONSUL, "Insufficient privileges to grant roles.");
            require(rank <= AuthLib.Rank.CONSUL, "Cant assign a higher rank");
            // Grant the role to the specified account
            roleData.grantRole(account, rank);
            // Emit an event for logging
            emit RoleGranted(account, rank);
